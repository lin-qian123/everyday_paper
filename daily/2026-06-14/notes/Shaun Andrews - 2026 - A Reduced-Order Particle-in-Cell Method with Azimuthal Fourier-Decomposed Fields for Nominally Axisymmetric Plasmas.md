...
- 文章的关键价值在于它不是牺牲掉非轴对称物理，而是专门针对“整体近似轴对称、但存在低阶方位角不稳定性”的问题做压缩。文中报告在基准问题上与解析或长时参考模拟定量一致，同时计算成本约降低 46 倍。
...
- 从本仓库方向看，这篇论文的意义主要在 PIC 算法层：如果未来后续需要处理近轴对称的激光等离子体、束流或磁化等离子体问题，它提供了一个比全 3D PIC 更现实的中间层方案。
...
- 如果后续想把它纳入自己的数值路线，优先判断本仓库的目标问题是否满足“整体近轴对称但局部有方位角不稳定性”的前提，再决定是否值得引入这一 reduced-order 框架。
