...
- 虽然对象是 stellarator 而不是激光等离子体，但它非常契合“AI/ML 用于物理约束优化”这一主线。特别是“先用物理数据重参数化，再做 BO”这一步，对 PIC、HEDP 实验设计甚至强场 QED 参数扫描都有方法学借鉴价值。

## 4. 可复现实验/仿真要点
- 真正值得复现的不是 BO 本身，而是数据驱动参数化这一步：把已有物理可行样本映射成更均匀、低维且带天然边界的搜索空间。
- 若迁移到你关心的方向，可以考虑把已有 PIC 仿真数据库、束流参数数据库或激光靶参数数据库先做同类重参数化，再把 surrogate / BO 接进去，而不是直接在原始高维工程参数上硬搜。

## 5. 后续行动项
- 这篇适合放到“AI for plasma/fusion optimization”子线，和近期深度学习 runaway electron、FNO for PIC、differentiable ICF design 一起看。
- 如果后面要整理一条“物理优化中的 AI 方法谱系”，这篇可以作为“物理可行域重参数化 + BO”的代表。
