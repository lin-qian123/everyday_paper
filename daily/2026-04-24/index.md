...
- 与方向关系：Strong-field radiation + spin-resolved PIC
...
- 与方向关系：HEDP warm dense matter

### 4) All-optically controllable electron and X-ray sources from microchannel-guided direct laser acceleration
...
- 与方向关系：Laser plasma acceleration + X-ray source + 3D PIC

### 5) Pushing the frontiers of light: magnetized plasma lenses and chirp tailoring for extreme intensities
...
- 与方向关系：Magnetized plasma lens + OSIRIS PIC

### 6) Laguerre-Gaussian pulses for spin-polarized ion beam acceleration
...
- 与方向关系：Spin-polarized ion acceleration + 3D PIC

## 检索与去重执行记录

- 已尝试检索来源：Crossref、OpenAlex、arXiv；当前环境均失败（代理不可达 + 直连 DNS 解析失败）。
- 已读取 `state/processed_articles.json`（50 条）作为去重基线。
- 今日候选来自近两日未入库且高相关正式论文重试池，共 6 篇，均未命中 state 去重键。

## 下载与校验状态

- 10.1017/hpl.2025.10093：下载失败；原因：代理 127.0.0.1:1087 不可达 + 直连 DNS 解析失败（cambridge.org/doi.org）。
- 10.1017/hpl.2026.10121：下载失败；原因：代理 127.0.0.1:1087 不可达 + 直连 DNS 解析失败（cambridge.org/doi.org）。
- 10.1017/hpl.2026.10108：下载失败；原因：代理 127.0.0.1:1087 不可达 + 直连 DNS 解析失败（cambridge.org/doi.org）。
- 10.1017/hpl.2025.10096：下载失败；原因：代理 127.0.0.1:1087 不可达 + 直连 DNS 解析失败（cambridge.org/doi.org）。
- 10.1017/hpl.2025.10101：下载失败；原因：代理 127.0.0.1:1087 不可达 + 直连 DNS 解析失败（cambridge.org/doi.org）。
- 10.1017/hpl.2025.10098：下载失败；原因：代理 127.0.0.1:1087 不可达 + 直连 DNS 解析失败（cambridge.org/doi.org）。
...
- `state/daily_retry_candidates.json`：已记录 2026-04-24 的 6 条候选失败上下文（DOI/来源/失败原因/重试命令）。

## 当日总结

- 今日主题覆盖 LWFA→PWFA 模式过渡、强场辐射极化调控、温稠密物质、磁化等离子体透镜与极化离子加速。
- 由于网络故障，今天未形成可入库增量；网络恢复后优先重试本页 6 篇正式论文。
