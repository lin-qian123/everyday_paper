...
- 已维护到 `daily/2026-09-26/`。本轮从 2026-09-25 官方 arXiv 新论文列表新增 `10.48550/arXiv.2609.28788`、`10.48550/arXiv.2609.29325`、`10.48550/arXiv.2609.28730` 与 `10.48550/arXiv.2609.29435`，分别覆盖 LWFA 电子—铅转换靶—GeV 缪子成像、激光/脉冲星等离子体一阶 QED Monte Carlo、恒星器湍流输运 AI 代理与闭环优化、暖稠密铝 mDFT 状态方程和输运。
- 4 份官方 PDF 均通过 `%PDF-`、`file`、`pdfinfo`、SHA-256 与非空 `pdftotext -layout`；MinerU 四项均返回 `parsing failed`，故使用本地布局文本、页面渲染和人工逐图检查 fallback。本轮保留并查看 16 张关键图，台账从 378 增至 382 条，17 条结构化重试项不变。
- 证据边界：缪子成像的空间信号为直接实验，但粒种占比和能区主要依赖 GEANT4/传播阈值；QED 采样尚未接入生产 PIC，也未报告端到端加速；AI-GX 的温度提升是数值闭环，最高代理分数反而对应 OOD 非物理解；mDFT 的最大高压分歧尚无对应实验锚点，`10 kT` 输运是后处理模型。本轮没有重跑 GEANT4、QED-PIC、GX/T3D、神经网络训练或 mDFT。
...
- 已维护到 `daily/2026-09-23/`。本轮新增正式 *Physical Review E* `10.1103/d45l-hsgg`、正式 *Physical Review D* `10.1103/f9xt-jpd1` 与 arXiv `10.48550/arXiv.2609.23358`，分别覆盖锥壁坍缩定时控制激光质子束能散/发散、辐射相对论 Alfvén 湍流中的粒子—光子各向异性，以及带 Poisson/电路重建、重锚和回滚的 ML 时间跳跃。前者曾因 APS 常规全文路径受阻，本轮从 APS harvest 正式端点恢复并首次入库。
- 3 份全文通过 `%PDF-`、`file`、`pdfinfo`、SHA-256 和非空 `pdftotext -layout`，均完成 MinerU；台账从 369 增至 372 条，18 条结构化重试项不变，本轮保留并逐张查看 14 张关键图。
- 证据边界：两篇 APS 都是作者 EPOCH PIC 模拟而非实验，`63%` 能散改善、`46%` 焦斑半径缩小和各向异性幂律均受当前模拟基线与二维/三维设置约束；PhySKIP 是一维氮气漂移—扩散流体求解器，不是 PIC，`4.70×` 为单逻辑 CPU、排除加载/诊断的作者计时。本轮没有本地运行 EPOCH、PASCHEN-1D、CNN 训练、粒子相空间或探测器响应。
...
- 已连续维护到 `daily/2026-09-17/`。本轮新增 `10.48550/arXiv.2609.16575`、`10.48550/arXiv.2609.17162` 与 `10.48550/arXiv.2609.17209`，分别覆盖 KEMPIC-3D 电磁 PIC 框架的分层验证、LAPD 强非线性 Alfvén 波实验—Hall-MHD—混合 PIC 机制链，以及 EKR/等变神经网络/学习型 Padé 动理学闭合。正式 *Physics of Plasmas* `10.1063/5.0347124` 与 2026-06-14 已入库的 `arXiv:2606.04887` 标题完全相同，按硬去重排除。
- 3 份官方 arXiv PDF 通过 `%PDF-`、`file`、`pdfinfo`、SHA-256、非空 `pdftotext -layout` 与 MinerU Markdown 转换；台账从 354 增至 357 条，16 条结构化重试项不变。本轮保留并逐张查看 12 张关键图。
- 证据边界：KEMPIC 的 LWFA 是 `Ny=2` 的准二维短程作者算例且源码尚未实际发布；Alfvén 工作直接测得非线性新模和尺度转移，但“有限 $k_\perp d_i$ 机制”还依赖量级标度与混合 PIC，且未形成充分发展湍流；非线性闭合只在解析线性矩层级、bump-on-tail 和 slab-ITG 模型上验证，神经闭合尚无一般因果和非线性长期稳定保证。本轮没有本地运行 KEMPIC、CAMELIA、Vlasov 或闭合代码。

## 当前状态（2026-09-16）

- 已连续维护到 `daily/2026-09-16/`。本轮新增 arXiv `10.48550/arXiv.2609.15968`、`10.48550/arXiv.2609.15043` 与正式 PRE `10.1103/f237-bqz2`，分别覆盖高电荷 LWFA 束团出口 electron shedding 的 FREM—FBPIC 证据链、ICP 分布式感应/电容反馈的 phasor–PIC/MCC 算法，以及 Fe/Ni 不透明度的 CNN–MLP–FiLM 代理。
- 3 份官方 arXiv 全文通过 `%PDF-`、`file`、`pdfinfo`、SHA-256、非空 `pdftotext -layout` 与 MinerU Markdown 转换；台账从 351 增至 354 条，16 条结构化重试项不变。当日保留并逐张查看 12 张关键图。PRE 论文正式发表于 2026-07-06，本轮仅因 2026-09-15 目标分类交叉列出而补入，未把交叉列表日期写成新出版日期。
- 证据边界：electron shedding 的 FREM 场印迹是实验，头—尾能量转移、`2 MeV` 重建阈值与近 `20%` 主束能量损失来自作者 PIC/合成诊断；phasor–PIC 的高精度制造解主要是无粒子线性验证，GEC 是二维作者模拟与功率口径不完全相同的历史探针对照；不透明度网络只插值 ILIade 标签，未做跨代码、实验、OOD 或端到端速度验证。本轮没有本地重跑 FBPIC、PIC/MCC、原子物理或神经网络训练。

## 当前状态（2026-09-15）

- 已连续维护到 `daily/2026-09-15/`。本轮新增正式接收 PRL `10.1103/mbn4-fd4v` 与正式 HPLSE `10.1017/hpl.2026.10174`，分别覆盖 DIII-D helicon 电流驱动实验，以及低密度泡沫靶冲击的时间分辨 X 射线照相—MULTI/FLASH 流体模拟链。
- 2 份开放全文通过 `%PDF-`、`file`、`pdfinfo`、SHA-256、非空 `pdftotext -layout` 与 MinerU Markdown 转换；台账从 349 增至 351 条。helicon 的合法 arXiv 作者稿补回此前 APS `HTTP 403` 阻塞项，重试队列从 17 降至 16 条；当日保留并逐张查看 17 张关键图。
- 证据边界：helicon 的 `20 ± 10 kA` 是 MSE/EFIT 总电流差扣除模型化欧姆、bootstrap、NBI 和 ECCD 电流后的残差，单侧天线未做 $k_\parallel$ 反转，低比压 L 模原理验证不能外推为反应堆效率；泡沫实验直接约束推动层位置和 `18.5 ± 2 km/s` 速度，而 `(3–5)×10¹³ W/cm²` 有效强度与约 `1 Mbar` 压力来自流体模型，均匀泡沫近似不能外推到直接照射的欠临界微结构泡沫。

## 当前状态（2026-09-14）

- 已连续维护到 `daily/2026-09-14/`。本轮新增官方 arXiv `10.48550/arXiv.2609.10958` 与正式 PRL `10.1103/82y9-svrd`，分别覆盖空心阴极羽流的实验—WarpX—粒子追踪机制链，以及 $^{50}$Ti M1 强度的实光子/电子/质子/单中子转移多探针实验。
- 2 份官方 arXiv 全文通过 `%PDF-`、`file`、`pdfinfo`、SHA-256、非空 `pdftotext -layout` 与 MinerU Markdown 转换；台账从 347 增至 349 条，17 条来源限制重试项不变。5 条 APS 高优先级候选再次由 accepted/article 与 DOI 路径返回 `HTTP 403`，已更新重试计数；当日保留并逐张查看 13 张关键图。
- 证据边界：Wang 的 RPA/探针结果是实验，但 WarpX 是代表性二维轴对称静电机制算例，完整/平均/冻结/仅涨落场是不可相加的敏感性对照，双探针未分辨连续离子声支也不等于排除离子声活动；Kelly 的四探针结论是正式核结构实验，但谱因子依赖 ADWA，旧 $(p,p')$ 局部截面含图像数字化系统学，三个模型的总强度相符建立在常规淬灭或参数拟合上。

## 当前状态（2026-09-13）

- 已连续维护到 `daily/2026-09-13/`。本轮新增官方 arXiv `10.48550/arXiv.2609.11628`、`10.48550/arXiv.2609.11844`、`10.48550/arXiv.2609.07793`，分别覆盖 TJ-II 稀疏剖面的 inverse PINN 输运反演、结构化光阴极的周期单胞—有限束团耦合，以及同步辐射冷却等离子体的 firehose/SCI 两相演化。
- 3 份官方全文通过 `%PDF-`、`file`、`pdfinfo`、SHA-256、非空 `pdftotext -layout` 与 MinerU Markdown 转换；台账从 344 增至 347 条，17 条来源限制重试项不变。当日保留并逐张查看 12 张关键图。
- 证据边界：Gallego 的真实输入来自 TJ-II，但电导率是简化一维方程下的有效反演；Bazyl–Zagorodnov 只在可承担的小阵列做同模型全分辨率验证，52 万孔注入器没有全分辨率真值；Wierzchucka 的早期 PIC/流体交叉验证有效，主 PIC 在 `t≳2.2τ0` 后受数值碰撞污染，长期饱和由流体延拓且不属于强场 QED 实证。
...
- 已连续维护到 `daily/2026-09-09/`。本轮新增官方 arXiv `10.48550/arXiv.2609.03399`、`10.48550/arXiv.2609.01793` 与 `10.48550/arXiv.2609.01750`，分别覆盖电子束–强激光 nonlinear-Compton MCP 搜寻与 magnetar Schwinger 约束、真实氢质量比半碰撞 MRX 电子电流片模拟，以及 DIII-D/MAST 无磁诊断平衡重建机器学习挑战。
- 3 份全文通过 `%PDF-`、`file`、`pdfinfo`、SHA-256 和非空 `pdftotext -layout`；台账从 330 增至 333 条，12 条来源限制重试项不变。当前环境缺少 `MINERU_TOKEN`，因此没有声称完成 MinerU 转换；当日渲染、解码并人工查看 9 个关键 PDF 页面。
- 证据边界：Jiang 的 $\epsilon\sim10^{-8}$ / magnetar $\sim10^{-9}$ 是理论灵敏度与能损约束，不是 MCP 观测；Son 的 `0.744 cm / 6.26 d_e` 来自 2D VPIC 对 MRX 参数和探针采样的重现，$\rho_e$ 归一化仍差约 `3–6` 倍；Nakkina 等发布的是数据集、baseline 与竞赛协议，且摘要 `9,113/2,416` 与正文 release `8,794/2,414` shots 口径不同，不能写成已部署反应堆控制器。
...
- 台账条目默认按关键词分类；遇到 `inertial confinement` / `magnetic confinement` 等明显歧义时，可用 `categories` 字段列出合法分类 slug 做人工覆盖，索引脚本会校验未知 slug 并拒绝生成。
